...
description: Unity C# 코드 작성 및 수정 시 적용하는 스타일 규칙. C# 파일(.cs)을 생성하거나 수정할 때 사용. BSD 중괄호, 한국어 주석, region 구조, 네이밍 규칙 등을 포함. Use when writing, editing C# code files.
---
...
- `if`, `for`, `while`, `foreach` 등 모든 제어문은 단일 문장이라도 반드시 중괄호(`{ }`)를 사용
- `for (...) Code();`와 같이 한 줄로 작성하는 것을 금지하며, 반드시 줄바꿈 후 중괄호로 감싸야 함
...
- 인자가 많아 길어지는 경우, 호출 전 긴 변수를 짧은 지역 변수로 할당 (의미 유지. 예: `p` X → `position` O)
- 그래도 여러 줄이 되면 괄호를 중괄호처럼 사용. 메서드 이름 뒤 줄바꿈 후 `(` 배치, 연관 인자끼리 묶어서 표현
- 선언부도 동일하게 적용
